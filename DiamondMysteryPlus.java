...
class Players {
    ...
    public boolean hasMatchingCard(String characterGuess, String placeGuess, RoomCards roomGuess) {
        for (Cards card : hand) {
...
public class DiamondMysteryPlus {
    ...
    public static void main(String[] args) {
        ...
        while (!gameOver) {
            ...
            if (currentPlayer == realPlayer) {
                ...
                gameOver = simulateRandomPlayerTurn(currentPlayer, hiddenCharacterCard, hiddenPlaceCard, hiddenRoomCard, roomCards, players, firstRound);
            }
    ...
    private static boolean simulateRandomPlayerTurn(Players player, Cards hiddenCharacterCard, Cards hiddenPlaceCard, RoomCards hiddenRoomCard, List<RoomCards> roomCards, List<Players> players, boolean firstRound) {
        ...
        boolean finalGuessDecision = new Random().nextBoolean();
        for (Players otherPlayer : players) {
            if (otherPlayer != player) {
                boolean hasMatch = otherPlayer.hasMatchingCard(randomCharacterGuess, randomPlaceGuess, randomRoomGuess);
                if (hasMatch) {
                    System.out.println(otherPlayer.getName() + " has a matching card.");
                    if (finalGuessDecision) {
                        return makeFinalGuess(hiddenCharacterCard, hiddenPlaceCard, hiddenRoomCard, randomCharacterGuess, randomPlaceGuess, randomRoomGuess);
                    } else {
                        return false;
                    }
                }
    ...
    private static boolean makeFinalGuess(Cards hiddenCharacterCard, Cards hiddenPlaceCard, RoomCards hiddenRoomCard, String finalCharacterGuess, String finalPlaceGuess, RoomCards finalRoomGuess) {
        if (hiddenCharacterCard.getName().toLowerCase().equals(finalCharacterGuess.toLowerCase()) &&
                hiddenPlaceCard.getName().toLowerCase().equals(finalPlaceGuess.toLowerCase()) &&
                hiddenRoomCard.getRoomNo() == finalRoomGuess.getRoomNo()) {
            System.out.println("Congratulations! The correct guess was made!");
            return true;
        } else {
            System.out.println("Incorrect guess. The correct answer was:");
            System.out.println( hiddenCharacterCard.getDetails());
            System.out.println(hiddenPlaceCard.getDetails());
            System.out.println( hiddenRoomCard.getDetails());
            return true;
        }
    }
